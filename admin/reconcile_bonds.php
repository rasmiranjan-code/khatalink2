...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['settle_id'])) {
    $pdo->prepare("UPDATE bond_payments SET is_settled_manually = ? WHERE id = ?")->execute([(int)$_POST['status'], (int)$_POST['settle_id']]);
    header("Location: reconcile_bonds.php?success=Bond status updated.");
    exit();
}

$query = "SELECT bp.*, s.shop_name, s.bank_acc_no, s.bank_ifsc, c.name as customer_name
...
foreach($payments as $p) {
    $base = (float)$p['amount_paid'];
    $gross = $base * (1 + (BOND_PLATFORM_COMMISSION_PERCENT / 100));
    ...
    if ($p['is_settled_manually'] && date('Y-m-d', strtotime($p['payment_date'])) == date('Y-m-d')) {
        $settled_today++;
    }
}
?>
...
<head>
    ...
    <title>Bond Settlements — KhataLink Admin</title>
...
<div class="layout">
    ...
    <main class="main">
         <div class="page-header">
            <div class="page-title">Bond Settlements</div>
            <div class="page-subtitle">Reconcile security bond installments held by the platform for unlinked merchants.</div>
        </div>
        ...
        <div class="stat-grid">
            ...
            <div class="stat-card shadow-sm">
                ...
                <div class="stat-label">Our Profit (1%)</div>
                 ...
                 <table class="kl-table">
                    ...
                    <tbody>
                        <?php foreach($payments as $p): 
                            $payout = (float)$p['amount_paid'];
                            $comm = $payout * (BOND_PLATFORM_COMMISSION_PERCENT / 100);
                        ?>
                        <tr>
                            <td>
                                 <span class="badge-uid" style="font-size:9px;">BOND</span>
                                <div class="text-muted mt-1 small"><?= date('d M Y', strtotime($p['payment_date'])) ?></div>
                            </td>
                            <td>
