...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['settle_id'])) {
    $pdo->prepare("UPDATE payment_requests SET is_settled_manually = ? WHERE id = ?")->execute([(int)$_POST['status'], (int)$_POST['settle_id']]);
    header("Location: reconcile_ledger.php?success=Ledger status updated.");
    exit();
}

$shop_id_filter = (int)($_GET['shop_id'] ?? 0);

$query = "SELECT pr.*, s.shop_name, s.bank_acc_no, s.bank_ifsc, c.name as customer_name, c.unique_id as customer_uid, s.id as shop_id
...
foreach($payments as $p) {
    $base = (float)$p['amount'];
    $gross = $base * (1 + (LEDGER_PLATFORM_COMMISSION_PERCENT / 100)); // e.g. 3.09
...
<head>
    ...
    <title>Ledger Settlements — KhataLink Admin</title>
...
<div class="layout">
    ...
    <main class="main">
        <div class="page-header">
            <div class="page-title">Ledger Settlements</div>
            <div class="page-subtitle">Reconcile daily udhar payments held by the platform for unlinked merchants.</div>
        </div>

        <div class="stat-grid">
            <div class="stat-card shadow-sm">
                <div class="stat-icon" style="background:#ecfdf5; color:#059669;"><i class="fas fa-hand-holding-dollar"></i></div>
                <div class="stat-label">Net Payout Pending (To Shops)</div>
            ...
            <div class="stat-card shadow-sm">
                ...
                <div class="stat-label">KL Profit (After PG Fees)</div>
        ...
        <!-- Shop Filter -->
        <div class="kl-card mb-4 p-3">
            <form method="GET" class="row g-3 align-items-center">
                <div class="col-auto">
                    <label class="kl-label mb-0">Filter by Shop:</label>
                </div>
                <div class="col-md-4">
                    <select name="shop_id" class="kl-input" onchange="this.form.submit()">
                        <option value="0">All Registered Shops</option>
                        <?php foreach($all_shops as $sh): ?>
                            <option value="<?= $sh['id'] ?>" <?= $shop_id_filter == $sh['id'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($sh['shop_name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-auto">
                    <a href="reconcile_ledger.php" class="btn btn-sm btn-outline-secondary rounded-3">Clear</a>
                </div>
            </form>
        </div>
                ...
                <table class="kl-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-end">Gross Received</th>
                            <th class="text-end">PG Charges</th>
                            <th class="text-end">Merchant Payout</th>
                            <th class="text-end">KL Profit</th>
                    ...
                    <tbody>
                        <?php foreach($payments as $p): 
                            $base = (float)$p['amount'];
                            $gross = $base * (1 + (LEDGER_PLATFORM_COMMISSION_PERCENT / 100));
                            $pg_fee = $gross * (PG_FEE_PERCENT / 100);
                            $payout = $base;
                            $profit = $gross - $pg_fee - $payout;
                        ?>
                        <tr>
                            <td>
                                <span class="badge-uid" style="font-size:9px;">LEDGER</span>
                            ...
                            <td class="text-end">
                                ...
                                <div class="text-[8px] text-slate-400 uppercase"><?= PG_FEE_PERCENT ?>%</div>
                            </td>
                            <td class="text-end">
                                <div class="fw-black text-emerald-600">₹<?= number_format($payout, 2) ?></div>
                                <div class="text-[8px] text-slate-400 uppercase">100% Base</div>
                            </td>
                            <td class="text-end">
