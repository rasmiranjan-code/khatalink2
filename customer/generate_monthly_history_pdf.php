...
if ($user_type === 'shop') {
    // Shop viewing monthly khata history
    $query = "
    ...
    $title_type = 'Monthly Khata History';

} else {
    // Customer viewing their monthly khata history
    $query = "
    ...
    $title_type = 'Your Monthly Khata Cycles';
}
...
<head>
    ...
    <style>
       ...
       .status-badge { font-weight: 700; padding: 4px 8px; border-radius: 6px; font-size: 10px; }
       .status-settled { background: #d1fae5; color: #059669; }
       .status-open { background: #fee2e2; color: #dc2626; }
       .mode-cash { color: #d97706; font-weight: 600; }
       .mode-online { color: #2563eb; font-weight: 600; }
...
<body>
    ...
    <div class="header">
        <div class="logo-section">
            ...
            <div class="shop-info">
                <h1 class="main-title">KhataLink Monthly History</h1>
                <p class="subtitle"><?= $title_type?></p>
            </div>
        </div>
        <div class="customer-info" style="text-align: right;">
            <h3><?= htmlspecialchars($title_name)?></h3>
            <p>Date: <?= date('d M Y')?></p>
            <p>Period: <?= date('d M Y', strtotime($from_date))?> - <?= date('d M Y', strtotime($to_date))?></p>
            <p>Filter: <?= htmlspecialchars($filter_name)?></p>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            foreach($cycles as $mk):
                ...
                $payment_mode_class = $is_manual? 'mode-cash' : 'mode-online';
                ...
                $display_sub = $user_type === 'shop'? $mk['unique_id'] : $mk['shop_category'];
          ?>
            <tr>
                ...
                <td class="text-center">
                    <span class="status-badge <?= $is_paid? 'status-settled' : 'status-open'?>">
                        <?= $is_paid? 'SETTLED' : 'OPEN'?>
                    </span>
                </td>
                <td class="text-center">
                    <span class="<?= $payment_mode_class?>">
